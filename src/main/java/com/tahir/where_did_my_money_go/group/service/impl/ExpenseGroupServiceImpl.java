...
import com.tahir.where_did_my_money_go.group.dto.*;
...
public class ExpenseGroupServiceImpl implements ExpenseGroupService {
                ...
                List<GroupBalanceResponseDTO.UserBalanceDTO> balances = netBalances.entrySet().stream()
                                .map(e -> {
                                        ...
                                        return GroupBalanceResponseDTO.UserBalanceDTO.builder()
                                                        .userId(e.getKey())
                                                        .name(userRepository.findById(e.getKey()).orElseThrow()
                                                                        .getName())
                                                        .netBalance(net)
                                                        .owes(net.compareTo(BigDecimal.ZERO) < 0 ? net.abs()
                                                                        : BigDecimal.ZERO)
                                                        .owed(net.compareTo(BigDecimal.ZERO) > 0 ? net
                                                                        : BigDecimal.ZERO)
                                                        .build();
                                }).toList();
        ...
        public List<GroupActivityResponseDTO> getGroupActivities(UUID groupId, UUID userId) {
                ...
                List<GroupActivityResponseDTO> expenseDTOs = expenses.stream()
                                .map(e -> {
                                        LocalDateTime dateTime = LocalDateTime.of(
                                                        e.getExpenseDate(),
                                                        e.getCreatedAt().toLocalTime());

                                        return GroupActivityResponseDTO.builder()
                                                        .type("EXPENSE")
                                                        .id(e.getId())
                                                        .groupId(groupId)
                                                        .amount(e.getTotalAmount())
                                                        .description(e.getDescription())
                                                        .fromUserId(e.getCreatedBy().getId())
                                                        .toUserId(null)
                                                        .date(dateTime)
                                                        .build();
                                })
                ...
                List<GroupActivityResponseDTO> settlementDTOs = settlements.stream()
                                .map(s -> GroupActivityResponseDTO.builder()
                                                .type("SETTLEMENT")
                                                .id(s.getId())
                                                .groupId(groupId)
                                                .amount(s.getAmount())
                ...
                List<GroupActivityResponseDTO> merged = new ArrayList<>();
                merged.addAll(expenseDTOs);
                merged.addAll(settlementDTOs);

                return merged.stream()
                                .sorted(Comparator.comparing(GroupActivityResponseDTO::getDate).reversed())
                                .toList();
        }
